...
- **Player Core**: Shaka Player (tối ưu HLS delay thấp, tự động phát luồng dự phòng `http://bore.pub:30113/hls/index.m3u8` khi luồng chính lỗi) + nút **Phóng to** trên trang TV & cửa sổ player (Vừa khung / Phóng to lấp khung / Kéo giãn — lưu lựa chọn theo máy).
...
- **Bảo vệ luồng (chống rip m3u8)**: `/api/playlist` CHỈ trả metadata (không có `stream_url`), mọi file `.m3u/.m3u8/.mpd` static bị chặn 404, client phải gọi `/api/stream/token` (JWT) — server kiểm tra đăng nhập + gói cước + quota xem thử rồi mới trả URL phát. **Từ 2026-09-13: chế độ `STREAM_MODE=direct` (mặc định) — server trả URL gốc, app phát trực tiếp; phù hợp vì kênh premium đã chuyển hết sang DASH `.mpd` (proxy không rewrite được MPD XML nên đi proxy là vỡ) và không tốn băng thông Worker. ĐÁNH ĐỔI: link gốc lộ trên DevTools/sniffer, lớp mã hoá AES-128 của proxy (`PROTECT=on`) không còn tác dụng** — các lớp kiểm tra đăng nhập/gói cước/xem thử vẫn nguyên. Muốn giấu link trở lại: `STREAM_MODE=auto` (proxy trước, tự tụt direct cho kênh `.mpd`/kênh bị nguồn chặn IP Cloudflare) hoặc `STREAM_MODE=proxy` (giấu tuyệt đối, chấp nhận kênh bị nguồn chặn không xem được). Chi tiết: `CHONG_RIP_STREAM.md`.
- **Token kênh .mpd (DASH)**: Admin → tab **"Token .mpd"** — chọn kênh có link `.mpd`, nhập token (vd `Ken1402@`) → khi phát server **tự ghép** `?token=…` vào URL manifest (`.../manifest.mpd?token=Ken1402@`), client không cần cấu hình gì. Token không trả về API công khai (chỉ hiện bản che `Ke•••2@`), không mất khi import lại M3U. Kênh `.mpd` ở chế độ auto luôn phát trực tiếp (proxy chưa rewrite được MPD XML). **Kênh `.mpd` cũng không gửi báo cáo lỗi shaka tự động** (shaka hay bắn lỗi RECOVERABLE vì token/segment trong khi hình vẫn chạy — client bỏ qua, server lọc thêm để APK cũ không làm bẩn tab "Lỗi player"), và health check ping đúng URL có `?token=` nên không còn bị gắn cờ "chết" oan. Chi tiết: `TOKEN_KENH_MPD.md` §6.
