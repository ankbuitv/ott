    ...
    <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#1a1c24] border border-slate-800/60 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 pt-5 pb-3 text-center relative">
          ...
          <div className="w-12 h-12 bg-[#f36f21]/15 rounded-2xl flex items-center justify-center mx-auto mb-3">
            <span className="text-[#f36f21] font-extrabold text-lg">C</span>
          </div>
        ...
        <div className="px-5 pb-5">
          ...
          {view === 'login' && (
            <form onSubmit={handleLogin} className="space-y-2.5">
              <div className="relative">
                ...
                <input type="text" value={loginVal} onChange={e => setLoginVal(e.target.value)} placeholder={t('auth.email_or_username')} required className="w-full pl-10 pr-3 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-[#f36f21]/60" />
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" />
                <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder={t('auth.password')} required className="w-full pl-10 pr-10 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-[#f36f21]/60" />
              ...
              <button type="submit" disabled={loading} className="w-full py-2.5 bg-[#f36f21] hover:bg-[#f36f21] disabled:opacity-50 text-white font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-1.5">
                {loading ? t('app.loading') : <>{t('auth.btn.login')} <ArrowRight className="w-4 h-4" /></>}
              </button>
              <div className="flex items-center justify-between text-[11px] pt-1">
                <button type="button" onClick={() => { setView('forgot'); resetForm(); }} className="text-[#ff9a3d] hover:text-[#ffb37a]">{t('auth.link.forgot')}</button>
          ...
          {view === 'register' && (
            <form onSubmit={handleRegister} className="space-y-2.5">
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" />
                <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder={t('auth.username')} required className="w-full pl-10 pr-3 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-[#f36f21]/60" />
              </div>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('auth.email')} required className="w-full pl-10 pr-3 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-[#f36f21]/60" />
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" />
          ...
          {view === 'verify' && (
            <form onSubmit={handleVerify} className="space-y-2.5">
              ...
              <button type="submit" className="w-full py-2.5 bg-[#f36f21] hover:bg-[#f36f21] text-white font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-1.5">
          ...
          {view === 'reset' && (
            <form onSubmit={handleReset} className="space-y-2.5">
              ...
              <button type="submit" className="w-full py-2.5 bg-[#f36f21] hover:bg-[#f36f21] text-white font-bold text-sm rounded-xl transition-all">{t('auth.btn.reset')}</button>
            </form>
          )}
        </div>
      </div>
