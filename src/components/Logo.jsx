...
// ===== Logo mark mới: viên play cam gradient trên nền tối, viền sáng tinh tế =====
...
// Logo CHRTV PLAY — gọn, hiện đại
export default function Logo({ size = 'md', showSubtext = true, className = '' }) {
  const { t } = useI18n();
  const sizes = {
    sm: { main: 'text-[15px]', play: 'text-[8px] px-1.5 py-0.5', sub: 'text-[7px]', icon: 'w-8 h-8' },
    md: { main: 'text-xl', play: 'text-[9px] px-2 py-[3px]', sub: 'text-[8px]', icon: 'w-10 h-10' },
    lg: { main: 'text-[26px]', play: 'text-[10px] px-2 py-1', sub: 'text-[9px]', icon: 'w-12 h-12' },
    xl: { main: 'text-4xl', play: 'text-xs px-2.5 py-1', sub: 'text-[11px]', icon: 'w-16 h-16' },
  }[size] || { main: 'text-xl', play: 'text-[9px] px-2 py-[3px]', sub: 'text-[8px]', icon: 'w-10 h-10' };
    ...
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      <div className="flex flex-col leading-none">
        <span className="flex items-center gap-1.5">
          ...
          <span className={`${sizes.play} font-black tracking-[0.18em] text-white rounded-md grad-brand shadow-lg shadow-[#f36f21]/30`}>PLAY</span>
        </span>
        {showSubtext && (
