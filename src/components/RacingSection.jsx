...
import { Flag, CalendarDays, ListOrdered, Trophy, ChevronDown } from 'lucide-react';
import { useI18n } from '../contexts/I18nContext';
import { fetchF1Schedule, fetchF1Standings, fetchF1Results, fetchMotorsport, f1Season } from '../services/racing';
...
function RaceCard({ race, winner }) {
  ...
  return (
    <div className={`rounded-2xl border p-3 ${upcoming ? 'bg-white/[0.03] border-white/[0.07]' : 'bg-white/[0.015] border-white/[0.05]'}`}>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[10px] font-black uppercase tracking-widest text-stone-500">R{race.round}</span>
...
export default function RacingSection() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!on) return;
      setSched(s || []); setStand(st || { drivers: [], teams: [] }); setMoto(m || []);
      setLoading(false);
