      ...
      <div className="flex items-center gap-1.5 md:gap-2">
        ...
        <div className="relative" ref={notifRef}>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-[#f36f21] text-white text-[9px] font-bold flex items-center justify-center">
          ...
          {notifOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 max-w-[92vw] bg-[#141419] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
              ...
              <div className="max-h-72 overflow-y-auto">
                {notifs.length === 0 ? (
                  <p className="px-4 py-6 text-center text-[11px] text-stone-500">{t('nav.no_notifs')}</p>
                ) : notifs.map((n) => (
              ...
              <button onClick={togglePush} className="w-full px-4 py-2.5 text-[11px] font-bold text-left border-t border-white/5 hover:bg-white/5 flex items-center gap-2">
                ...
                {pushOn ? t('nav.push_on') : t('nav.push_off')}
              </button>
            </div>
          )}
        </div>
