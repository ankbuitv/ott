    ...
    <nav className="topbar-mytv px-4 md:px-6 h-16 flex items-center justify-between sticky top-0 z-40 shrink-0">
      <div className="flex items-center gap-6">
        ...
        <div className="relative hidden md:block" ref={boxRef}>
          <div className={`flex items-center gap-2 bg-white/5 hover:bg-white/10 border ${searchFocused ? 'border-[#f36f21]' : 'border-white/10 hover:border-white/20'} px-3.5 py-2 rounded-xl w-80 transition-all`}>
            ...
            <input className="bg-transparent text-sm text-white placeholder:text-stone-500 focus:outline-none flex-1" placeholder={t('app.search.placeholder')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setSearchFocused(true)} onKeyDown={e => { if (e.key === 'Escape') setSearchFocused(false); }} />
              ...
              {channelMatches.length > 0 && (
                <div className="p-2">
                  ...
                  {channelMatches.map(ch => (
                    <button key={ch.channel_id} onClick={() => { setSearchQuery(''); setSearchFocused(false); onSelectChannel && onSelectChannel(ch); }} className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-white/5 transition text-left">
