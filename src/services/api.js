/**
 * CHRTV - Backend Service Integration
...
/**
 * Tải danh sách kênh từ API Worker hoặc nguồn dự phòng
 */
export async function fetchChannels() {
  try {
    ...
    console.warn("Lỗi khi kết nối Worker Playlist, chuyển sang nguồn kênh mặc định:", err.message);
  }

  // Danh sách kênh mặc định nếu API Worker không phản hồi
  return [
    {
      channel_id: "VTV1.vn",
      name: "VTV1 HD",
...
export async function fetchEPGData() {
  try {
    ...
    console.warn("Lỗi khi kết nối Worker EPG:", err.message);
  }
  return null;
}

/**
 * Tạo URL Proxy cho stream HLS
 */
...
/**
 * Tải danh sách kênh yêu thích từ D1 Database hoặc LocalStorage
 */
...
export async function toggleFavoriteApi(channelId, isFav) {
  ...
  try {
    ...
    console.warn("Lỗi đồng bộ Favorites D1:", e.message);
  }
...
export async function recordWatchHistory(channelId, position = 0) {
  ...
  try {
    ...
    console.warn("Lỗi đồng bộ Watch History D1:", e.message);
  }
}
